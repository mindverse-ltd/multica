...
## Tuning task timeouts

The daemon has its own execution limits before the server-side task sweeper gets involved:

| Setting | Default | CLI flag | Environment variable |
|---|---|---|---|
| Overall agent execution timeout | **2 hours** | `--agent-timeout` | `MULTICA_AGENT_TIMEOUT` |
| Codex semantic inactivity timeout | **10 minutes** | `--codex-semantic-inactivity-timeout` | `MULTICA_CODEX_SEMANTIC_INACTIVITY_TIMEOUT` |

The Codex timeout is provider-specific. It fires when Codex stops emitting meaningful progress; a still-running terminal command is treated as active work. If a Codex run fails around 10 minutes with a message like `codex semantic inactivity timeout`, raise `MULTICA_CODEX_SEMANTIC_INACTIVITY_TIMEOUT` on the machine running the daemon and restart the daemon.
