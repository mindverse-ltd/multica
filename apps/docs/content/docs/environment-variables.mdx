...
| Variable | Default | Description |
|---|---|---|
| `MULTICA_CODEX_CAPACITY_RETRY_COUNT` | `6` | Additional immediate retries when a task's persisted raw Codex error is exactly `Selected model is at capacity. Please try a different model.` The default allows seven total executions; `0` disables this dedicated policy. Maximum `20` — a negative, malformed or larger value stops the server at startup instead of falling back to the default, because every retry writes a task row, launches a runtime and calls the provider again. Other capacity or rate-limit errors are not retried by this policy. |
...
| `MULTICA_AGENT_TIMEOUT` | `2h` | Overall timeout for one agent task |
| `MULTICA_CODEX_SEMANTIC_INACTIVITY_TIMEOUT` | `10m` | Codex-only watchdog for no semantic progress; long-running terminal commands are treated as active |
