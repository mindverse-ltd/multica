.PHONY: help makehelp dev server daemon cli multica build test migrate-up migrate-down sqlc seed clean setup start stop check worktree-env setup-main start-main stop-main check-main setup-worktree start-worktree stop-worktree check-worktree db-up db-down db-reset selfhost selfhost-build selfhost-stop selfhost-native-backend selfhost-native-frontend selfhost-native-bootstrap selfhost-native-check selfhost-native-stop selfhost-feishu-preflight selfhost-feishu-configure
...
selfhost-native-backend: ## Start backend directly on the host using ENV_FILE
	$(REQUIRE_ENV)
	@bash scripts/selfhost-native-backend.sh "$(ENV_FILE)"

selfhost-native-frontend: ## Start frontend directly on the host using ENV_FILE
	$(REQUIRE_ENV)
	@bash scripts/selfhost-native-frontend.sh "$(ENV_FILE)"

selfhost-native-bootstrap: ## Bootstrap host-native self-host dependencies
	$(REQUIRE_ENV)
	@bash scripts/selfhost-native-bootstrap.sh

selfhost-native-check: ## Check host-native self-host services
	$(REQUIRE_ENV)
	@bash scripts/selfhost-native-check.sh "$(ENV_FILE)"

selfhost-native-stop: ## Stop host-native self-host services
	$(REQUIRE_ENV)
	@bash scripts/selfhost-native-stop.sh "$(ENV_FILE)"

selfhost-feishu-preflight: ## Validate Feishu self-host configuration
	$(REQUIRE_ENV)
	@bash scripts/selfhost-feishu-preflight.sh "$(ENV_FILE)"

selfhost-feishu-configure: ## Write Feishu self-host configuration into ENV_FILE
	$(REQUIRE_ENV)
